...
class Game {
    ...
    /** Arraylist of moves made. */
    private ArrayList<Game> moves = new ArrayList<Game>();
    ...
    void undoMove() {
        if (moves.size() > 1) {
            this.copyFrom(moves.get(moves.size() - 2));
            moves.remove(moves.size() - 1);
        } else if (moves.size() == 1){
            this.copyFrom(moves.get(0));
        }
    }
    ...
    /**
     * NEW METHOD gets the top card in the stock
     */
